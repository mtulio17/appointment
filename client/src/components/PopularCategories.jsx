...
const PopularCategories = () => {
  ...
  // Uso de react-query para obtener categorías
  const { data: categories, error, isLoading } = useQuery({
    queryKey: ['categories'],
    queryFn: getCategories,
    cacheTime: 1000 * 60 * 10, // Opcional: Mantener caché por 10 minutos
  });
          ...
          <Slider {...settings}>
            {trendCategories.map((category) => (
              ...
              >
                ...
                <h3 className="mt-5 lg:text-[14px] w-24 justify-center text-center font-semibold text-gray-800">
                  {category.name}
                </h3>
              </div>
            ))}
